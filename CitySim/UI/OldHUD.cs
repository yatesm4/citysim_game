...
namespace CitySim.UI
{
    public class OldHUD : Component
    {
        ...
        private PopupMenu _buildingsPopupMenu { get; set; }
        private Vector2 _popupCellDimensions { get; set; } = new Vector2(64, 64);

        private Texture2D _popupTexture { get; set; }
        private Vector2 _popupDimensions { get; set; } = new Vector2(216, (64 * CellMenu.CellRows) + (
                                                                              (CellMenu.CellRows + 1) * ((216 - (64 * 3)) / 4)));

        private Texture2D _popupCellTexture { get; set; }
        private Color _popupCellColor { get; set; }
        private Color[] _popupCellColorData { get; set; }
        ...
        public OldHUD(GraphicsDevice graphicsDevice_, GameContent content_)
        {
        ...
        public void SetColorData(GraphicsDevice graphicsDevice_)
        {
            ...
            var popupTexture = new Texture2D(graphicsDevice_, (int)_popupDimensions.X, (int)_popupDimensions.Y);
            var popupColorData = new Color[(int)_popupDimensions.X * (int)_popupDimensions.Y];
            ...
            var popupCellTexture =
                new Texture2D(graphicsDevice_, (int)_popupCellDimensions.X, (int)_popupCellDimensions.Y);
            var popupCellColorData = new Color[(int)_popupCellDimensions.X * (int)_popupCellDimensions.Y];
            for (int i = 0; i < popupCellColorData.Length; i++)
            {
                popupCellColorData[i] = Color.White;
            }
            popupCellTexture.SetData(popupCellColorData);
            _popupCellTexture = popupCellTexture;
        }
        ...
        public void SetButtons()
        {
            ...
            btnBuildings.Click += OpenBuildingsPopup;
            _components.Add(btnBuildings);

            // add popup menu for button
            var buildingsPopupPosition = init_btn_pos +
                                new Vector2(_buttonHouseTexture.Width / 2, _buttonHouseTexture.Height / 2) -
                                _popupDimensions;
            var buildingsPopupMenu = new BuildingMenu(_popupTexture, _font, _popupCellTexture, _content)
            {
                Position = buildingsPopupPosition,
                HoverColor = Color.Blue,
                Scale = new Vector2(1, 1)
            };
            _buildingsPopupMenu = buildingsPopupMenu;
            _components.Add(_buildingsPopupMenu);
        }

        private void OpenBuildingsPopup(object sender, EventArgs e)
        {
            Console.WriteLine($"{sender.ToString()} clicked! Popup is " + _buildingsPopupMenu.IsActive.ToString());
            _buildingsPopupMenu.IsActive = !_buildingsPopupMenu.IsActive.Equals(true);
        }
